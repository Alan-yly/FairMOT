...
from lib.mywork.mynetwork import Mynetwork
import torch
...
model2 = Mynetwork(config)
load_model(model2,'/home/hust/yly/Model/model_25.pth')
model2.cuda()
model2.eval()
...
for seq in seqs:
    ...
    for keysa in tqdm.tqdm(id_feat.keys()):
        ...
        inds = random.sample(range(l-2*seq_len+1),min(10,l-2*seq_len+1))
        for ind in inds:
            feata = []
            for i in range(seq_len):
                feata.append(id_feat[keysa][ind+i].squeeze())
            feata = torch.stack(feata).cuda().unsqueeze(0)
            jnds = random.sample(range(ind+seq_len,min(ind+599,l-seq_len+1)),min(10,l-2*seq_len-ind+1))
            for jnd in jnds:
                featb = []
                for i in range(seq_len):
                    featb.append(id_feat[keysa][jnd+i])
                featb = torch.stack(featb).cuda().unsqueeze(0)
                with torch.no_grad():
                    n = model2(feata,featb,torch.ones(1,30).cuda(),torch.ones(1,30).cuda()).cpu().numpy()
                if n > 1 or n < -1:
                    print('!'*100)
                hists[(jnd - ind) // frame_unint * frame_unint+ frame_unint].append(1-n)
