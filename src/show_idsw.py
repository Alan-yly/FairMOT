...
import pandas as pd
import numpy as np
data_root = 'G:\project\MOT\mot17_half_yolov5s'
    ...
    if file[-3:] == 'txt':
        ...
        for _,idsw in idsws.iterrows():
            ...
            if f:
                ...
                cv2.rectangle(img1, (track_tlbr[0], track_tlbr[1]), (track_tlbr[2], track_tlbr[3]), (0,255,0), 2)
                cv2.rectangle(img2, (last_track_tlbr[0], last_track_tlbr[1]), (last_track_tlbr[2], last_track_tlbr[3]),
                              (0,255,0), 2)

                cv2.rectangle(img1, (gt_tlbr[0], gt_tlbr[1]), (gt_tlbr[2], gt_tlbr[3]), (255,0,0), 2)
                cv2.rectangle(img2, (last_gt_tlbr[0], last_gt_tlbr[1]), (last_gt_tlbr[2], last_gt_tlbr[3]),
                              (255,0,0), 2)
